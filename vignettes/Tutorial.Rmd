...
- `dzmax` sets the maximum vertical (Z) distance allowed. In this tutorial, the Z value is based on DBH, so `dzmax = 0.03` means a 3 cm difference is allowed. For example, a tree measured in the field at 15 cm DBH cannot be matched with a LiDAR tree measured at 11 cm DBH, the cost would be infinite.
