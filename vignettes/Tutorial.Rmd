...
The user data can be in any format with various name comvention and units standard. Thus in order to get everything working we must standardize the data by telling which column contains X, Y coordinates as well as the DBH or tree height.
...
Then, for various reason, the algorithm need to know the center and the size of the plot we are dealing with. This is mostly useful for filtering data outside the limits. In this specific dataset the center is stored and repeated in columns `Easting` and `Northing`. Then we create the `TreeMap` object we are going to work with
...
To fix this, we can introduce dummy trees, and assign a constant cost to matching a real tree with a dummy one. This is exactly what the `unmatch_cost` parameter does. For example, if we set `unmatch_cost = 10000`, it means that matching a tree with a dummy is very expensive, so the algorithm will avoid it unless absolutely necessary.
...
- `dzmax` sets the maximum vertical (Z) distance allowed. In this tutorial, the Z value is based on DBH, so `dzmax = 0.1` means a 10 cm difference is allowed. For example, a tree measured in the field at 15 cm DBH cannot be matched with a LiDAR tree measured at 5 cm DBH, the cost would be infinite.

After setting reasonable values for these two parameters, the matching results now look much better.


```{r, fig.width=6, fig.height=6}
tm <- match_trees(tm, dxymax = 2, dzmax = 0.1, zrel = 40, unmatch_cost = 3)
plot(tm, scale = 2, gg = TRUE)
```
...
```{r, fig.width=6, fig.height=6}
tm <- match_trees(tm, dxymax = 2, dzmax = 0.1, zrel = 40)
plot(tm, scale = 2, gg = TRUE)
```
