...
#' @export
#' @md
lsap_matching = function(treemap, dxymax = 2, dzmax = 40, zrel = 40, unmatch_cost = "auto")
{
  inf = 9999999
  ...
  if (!is.numeric(unmatch_cost))
  {
    ...
    if (is.na(unmatch_cost))
      stop("Internal error: invalid guess for 'unmatch_cost")
  }
  ...
  d = as.numeric(sf::st_distance(inventory, treemap$center))
  inside = d < (treemap$radius + treemap$buffer)
  inventory$ZDIM[!inside] = inf
  ...
  ncol = max(0,nrow(d3)-ncol(d3)) + ncol(d3)
  unmatch = matrix(unmatch_cost, ncol = ncol, nrow = nrow(d3))
  d3 = cbind(d3, unmatch)
  ...
  if (length(invalid) > 0)
    match_table = match_table[-invalid,]
